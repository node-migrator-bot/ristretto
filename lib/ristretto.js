(function() {
  ...
  Ristretto = (function() {
    Ristretto.UnknownModule = (function() {
      __extends(UnknownModule, Error);
      function UnknownModule(req, fileName) {
        this.req = req;
        this.fileName = fileName;
        this.message = "Cannot find module `" + this.req + "` required in file `" + (path.basename(this.fileName)) + "`";
      }
    ...
    Ristretto.prototype.resolve = function(req, dir) {
      ...
      if ((fs.existsSync(reqPath)) && (fs.statSync(reqPath)).isDirectory()) {
        ...
        if (fs.existsSync(reqPath + '.coffee')) {
    ...
    Ristretto.prototype.add = function(fileName, moduleName) {
      ...
      module.body = this.load(fileName).replace(/require\(('|")(.*?)\1\)/g, __bind(function(_, _, req, pos, contents) {
        var dep, depName;
        depName = (function() {
          try {
            return this.resolve(req, path.dirname(fileName));
          } catch (e) {
            throw new Ristretto.UnknownModule(req, fileName);
          }
        }).call(this);
